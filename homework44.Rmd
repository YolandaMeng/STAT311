...
sd: C = (5-32) * 5$/$9 = -15Celsius
Z = (X-μ)$/$σ = (X-25)$/$(-15)
X~N[25, -15]

### b)

Z = (X-μ)$/$σ = (28-25)$/$(-15) =-0.2
P(Z>-0.2) = 1-P(Z<=-0.2) = 1-0.4207 = 0.5793
The probability is approximately 57.93%.

### c)

Part (a) in Exercise 4.7: Z = (X-μ)$/$σ = (83-77)$/$5 = 1.2
...
The population under consideration in the data set is all adults in the United States.
...
The statistic we use to measure the uncertainty of the point estimate is the standard error of the proportion.
...
Comparing the standard errors, we can see that the difference is small. This suggests that the point estimate is relatively stable even when using a slightly different assumed population proportion.
...
The distribution of sample proportions is called the sampling distribution of the sample proportion.

### b)

The shape is expected to be approximately symmetric. This is due to the Central Limit Theorem, which states that the distribution of sample means will be approximately normal if the sample size is sufficiently large, regardless of the shape of the population distribution. In this case, the sample size is 40, for repeating 1000 times, which is reasonably large, so we would expect the distribution to be approximately symmetric.
...
The formal name of the value computed in (c) is the standard deviation of the sample proportion.
...
The new variability of this distribution is 0.039, which is smaller than the variability of distribution when each sample contained 40 observations. This is because increasing the sample size reduces the standard deviation of the sample proportion, making the distribution narrower.
...
False. The statement about statistical significance is related to hypothesis testing. In this case, the statement is saying that the true proportion is more than half (52%). To test this, we would compare the observed proportion to a null hypothesis. The given data does not provide enough information about the null hypothesis or the test statistic, so we cannot determine statistical significance based on the information provided.

### b)

False. The standard error is a measure of the precision of the estimate, not the percentage of the population included in the study. The standard error of 2.4% indicates how much we expect the sample proportion to vary from the true population proportion due to random sampling.

### c)

False. The standard error is inversely proportional to the square root of the sample size. To reduce the standard error, we should collect more data (increase the sample size).


### d)

False. The confidence interval width is inversely proportional to the square root of the sample size. A 99% confidence interval will be wider than a 90% confidence interval because it needs to be more conservative to capture a higher percentage of possible sample means.

## Question 7 (Exercise 5.11 on Page 188): 

### a)

True. The 95% confidence interval (128 minutes, 147 minutes) indicates that we are 95% confident that the true population mean (average waiting time for all patients) falls within this interval.
...
True. The margin of error is directly proportional to the standard error, which is inversely proportional to the square root of the sample size. To cut the margin of error in half, you would need to quadruple the sample size.
